...
- **Atomicity** — each commit should pass tests on its own where practical. Check it concretely: would every intermediate tree type-check and test — including callers a later commit leaves on an old contract, and references to something a later commit introduces? Where it wouldn't, reorder or merge. Don't ship a feature commit whose tests live in a later commit.
