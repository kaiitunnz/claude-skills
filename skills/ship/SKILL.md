...
When `/loop-dev` invoked this skill, it has just established the branch and the base — take those as given in step 1 rather than re-deriving them. Its **verify result does not substitute for step 2**: that gate runs against the tree you are about to push, and `/loop-dev` verifies mid-implementation with commits landing after it. Skip step 2 only when the worktree is provably unchanged since that green run.
