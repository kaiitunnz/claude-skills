...
If `/loop-revise` isn't installed, do its work inline following the principles it names: critically self-review (a cold-context subagent running `/review-pr`, else `/review-diff <branch> <base>`, else inline); address findings with `/address-review` (fix what's right, push back with reasoning on what isn't, don't widen scope); re-verify, commit, and push each round; then run a final full verify as a hard gate before continuing. Loop until the review converges rather than to a fixed round count, and surface thrashing. Apply the same profile you'd have forwarded: under `thorough`, re-review every round and close with an unconditional full verify; under `lean`, converge on the first clean review, re-review only when a round changed code beyond a formatter reflow, and run the closing full verify only when code changed since the last full green. Resolve the profile the way `/loop-revise` does — explicit directive, else `lean` on Claude Opus 5 or newer, else `thorough`.
