...
- **Review profile.** The convergence loops (`loop-plan`, `loop-revise`, and the pipelines that drive them) take a `lean` / `thorough` directive. `thorough` keeps the belt-and-braces behavior these skills were written with — re-review every round, converge only after two quiet rounds, always re-run the closing gate. `lean` drops the repeated self-verification that a strongly self-correcting model already does on its own. It defaults to `lean` on Claude Opus 5 and newer and `thorough` everywhere else, including when the running model can't be determined, so the fallback is always the safer one. Either way, gates stay gates: thrashing halts, red halts, findings are addressed or explicitly pushed back.
