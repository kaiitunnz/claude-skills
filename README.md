...
- **A gate is not an endpoint.** Orchestrators (`ship`, `loop-dev`, `loop-revise`) finish on an endpoint or their own final report, never mid-pipeline — and a callee's "nothing else after that" bounds its output, not the caller's pipeline. A green `verify-impl` is a gate result and hands control back.
