...
- **A gate is not an endpoint.** Orchestrators (`ship`, `loop-dev`, `loop-revise`) finish on an endpoint — a PR URL, a branch, a file path — or an explicit halt. A green `verify-impl` is a gate result and hands control back.
